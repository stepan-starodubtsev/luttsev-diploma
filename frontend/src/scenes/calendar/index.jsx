// frontend/src/src/scenes/calendar/index.jsx
...
import 'dayjs/locale/uk'; // Переконайтесь, що локаль завантажена
...
import exerciseStore from "../../stores/exerciseStore"; // Для деталей вправ у модальному вікні
import useError from "../../utils/useError.js";
import { SessionTypes, ROLES } from "../../utils/constants.js";
import authStore from '../../stores/authStore.js';
import { tokens } from "../../theme.js"; // Для кольорів подій
...
const CalendarPage = observer(() => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            // Паралельне завантаження всіх необхідних даних
            await Promise.all([
    ...
    const formattedEvents = useMemo(() => {
        return trainingSessionStore.sessions.map(session => {
            ...
            let backgroundColor = colors.primary[500]; // Колір за замовчуванням
            let textColor = theme.palette.getContrastText(backgroundColor);

            if (session.session_type === 'STANDARDS_ASSESSMENT') {
                backgroundColor = colors.blueAccent[500] || theme.palette.info.main; // Використовуємо blueAccent
            ...
            // Отримуємо деталі вправ для модального вікна
            ...
            return {
                ...
                allDay: dayjs(session.end_datetime).diff(dayjs(session.start_datetime), 'hour') >= 23, // Приклад для allDay
                extendedProps: {
                    ...session, // Всі дані сесії
                    sessionTypeLabel,
                    unitName,
                    conductorName,
                    locationName,
                    exercisesDetails: eventExercises, // Передаємо деталізовані вправи
                },
                backgroundColor,
    ...
    const handleDateClick = (selectInfo) => {
        if (authStore.userRole === ROLES.ADMIN || authStore.userRole === ROLES.DEPARTMENT_EMPLOYEE || authStore.userRole === ROLES.COMMANDER) {
            ...
            // Передаємо початкову та кінцеву дату для створення
            // FullCalendar може повертати endStr на день більший для allDay слотів
            ...
            navigate("/training-sessions/create", {
                state: {
                    ...
                    endDate: endDate.hour(dayjs(selectInfo.startStr).hour() + 1).minute(dayjs(selectInfo.startStr).minute()).toISOString(), // Тривалість 1 година за замовчуванням
    ...
    return (
        <Box m="20px">
            ...
            <Paper elevation={3} sx={{ p: {xs: 1, sm: 2}, mt: 2 }}>
                <FullCalendar
                    locale={ukLocale}
                    height="auto" // Або конкретне значення, наприклад "75vh"
                    ...
                    editable={false} // Перетягування поки вимкнено для простоти
                    selectable={canManageEvents} // Дозволити вибір слотів тільки тим, хто може створювати
                    selectMirror={true}
                    dayMaxEvents={true} // Дозволяє "+ আরও" якщо багато подій
            ...
            {selectedEvent && (
                <Dialog open={isModalOpen} onClose={handleModalClose} maxWidth="sm" fullWidth scroll="paper">
                    ...
                    <DialogContent dividers>
                        <Typography variant="body1" gutterBottom><strong>Тип:</strong> {selectedEvent.extendedProps.sessionTypeLabel}</Typography>
                        <Typography variant="body1" gutterBottom><strong>Час:</strong> {dayjs(selectedEvent.start).format('DD.MM.YYYY HH:mm')} - {selectedEvent.end ? dayjs(selectedEvent.end).format('HH:mm') : 'N/A'}</Typography>
                        <Typography variant="body1" gutterBottom><strong>Локація:</strong> {selectedEvent.extendedProps.locationName}</Typography>
                        <Typography variant="body1" gutterBottom><strong>Проводить:</strong> {selectedEvent.extendedProps.conductorName}</Typography>
                        <Typography variant="body1" gutterBottom><strong>Підрозділ:</strong> {selectedEvent.extendedProps.unitName}</Typography>
