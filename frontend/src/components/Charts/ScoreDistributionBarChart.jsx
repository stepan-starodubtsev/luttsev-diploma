...
const ScoreDistributionBarChart = observer(() => {
    ...
    const [selectedUnitId, setSelectedUnitId] = useState('all'); // 'all' для відображення всіх
    ...
    useEffect(() => {
        ...
        if (militaryPersonnelStore.personnelList.length === 0 && !militaryPersonnelStore.loading) {
            militaryPersonnelStore.loadPersonnel(); // Це має завантажити і unitStore
        }
    ...
    const chartData = useMemo(() => {
        if (isLoading || !ScoreTypes || ScoreTypes.length === 0) return [];
    ...
    }, [standardAssessmentStore.assessments, ScoreTypes, unitStore.units, militaryPersonnelStore.personnelList, selectedUnitId, isLoading]);
    ...
    const getBarFillColor = (scoreValue) => {
        switch (scoreValue) {
            ...
            case 'GOOD': return chartColors.blueAccent[500];
            case 'SATISFACTORY': return chartColors.primary[400] || '#f2f0f0'; // З вашої теми
        ...
        <Paper elevation={3} sx={{ p: 2, height: '450px', display: 'flex', flexDirection: 'column' }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                <Typography variant="h6" gutterBottom component="div">
                    Розподіл Оцінок {selectedUnitId !== 'all' && unitStore.units.find(u => u.unit_id === parseInt(selectedUnitId)) ? `(Підрозділ: ${unitStore.units.find(u => u.unit_id === parseInt(selectedUnitId)).unit_name})` : '(Загалом)'}
                </Typography>
            ...
            {chartData.length > 0 ? (
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
                    <Typography variant="subtitle1">Немає даних для відображення для обраного фільтру.</Typography>
                </Box>
            )}
        </Paper>
    );
